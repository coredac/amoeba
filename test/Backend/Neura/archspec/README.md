...
To use this architecture specification in your tests, add the following option to your `mlir-amoeba-opt` command:

```bash
--architecture-spec=archspec/architecture.yaml
```

### Example

```bash
mlir-amoeba-opt input.mlir \
  --assign-accelerator \
  --lower-llvm-to-neura \
...
- From `test/`: `archspec/architecture.yaml`
- From `test/code_gen/`: `../archspec/architecture.yaml`
- From `test/neura/ctrl/`: `../../archspec/architecture.yaml`
