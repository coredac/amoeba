  ...
  static Architecture instance = []() {
    ...
    if (failed(architecture_result)) {
      llvm::report_fatal_error(
          "[mlir-amoeba-opt] Failed to get architecture.");
    }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg_ref == "--architecture-spec") {
      if (i + 1 < argc) {
        ...
        llvm::errs() << "[mlir-amoeba-opt] Error: --architecture-spec option "
                        "requires a value\n";
        return EXIT_FAILURE;
      }
  ...
  if (!architecture_spec_file.empty()) {
    llvm::errs() << "[mlir-amoeba-opt] Architecture specification file: "
                 << architecture_spec_file << "\n";
  } else {
    llvm::errs() << "[mlir-amoeba-opt] No architecture specification file "
                    "provided, using default configuration\n";
  }

  // Runs the MLIR optimizer.
  return mlir::asMainReturnCode(mlir::MlirOptMain(
      new_argc, new_argv, "Amoeba Dialect Optimizer", registry));
}
